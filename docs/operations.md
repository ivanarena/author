...
The Docker workflow builds as non-root, exports one image archive, scans it with a digest-pinned Trivy container without mounting the Docker socket, and publishes that exact scanned archive. A rebased `main` commit may reuse the reviewed archive only when the source tree, checksum, CI, and Security evidence match; otherwise it rebuilds and rescans. It fails on high/critical findings, emits and attests SBOM/provenance for the published digest, and signs it with keyless Cosign. Review the uploaded SARIF artifact from the originating scan before public deployments.
