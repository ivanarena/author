...
class NoteCrypto(
  ...
  fun encryptNotebookFields(
    ...
    return notebook.copy(nameHash = nameHash, name = encryptText(notebook.name, keyMaterial))
  }

  fun decryptNotebookFields(
    notebook: LocalNotebook,
    keyMaterial: String = getEncryptionKeyMaterial(),
  ): LocalNotebook = notebook.copy(name = decryptText(notebook.name, keyMaterial))
  ...
  private fun encryptionKey(keyMaterial: String): SecretKeySpec =
    SecretKeySpec(sha256("author:$keyMaterial".toByteArray(UTF_8)), "AES")
  ...
  private fun fieldHash(value: String, keyMaterial: String, context: String): String =
    "hash:v1:${base64UrlEncode(sha256("author-field-hash:$keyMaterial:$context\u0000$value".toByteArray(UTF_8)))}"
